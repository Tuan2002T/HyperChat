...
const verifyOTPRg = async (req, res) => {
  try {
  ...
  const registerUser = async (req, res) => {
      try {
        ...
        if (!userName || !password || !fullname || !birthday) return res.status(400).json({ error: "Bắt buộc nhập đầy đủ thông tin." });
...
const getListChats = async (req, res) => {
  try {
    const { userId } = req.params;
    const chatGroup = await userModel.findById(userId).populate('chatGroups', 'name members admin messages avatar updatedAt');
    const chatPrivate = await userModel.findById(userId).populate('chatPrivate', 'name members messages avatar updatedAt');
...
module.exports = { sendOTP, verifyOTPAndRegister, loginUser, getUsers, findUser, findUserByPhoneNumber, updateUser, listFriends, upload, sendOTPForgotPassword, verifyOTPForgotPassword, getListChats, changePassword, allFriendRequestSent
  ...
  , sendMailOTP, verifyOTPRg, registerUser
 };
